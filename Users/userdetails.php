<?php
    ...
    if(isset($_SESSION['login'][0]))
    {
        $acc=new Account();
        $order=new Orders();
        $acc=$acc->getUserById($_SESSION['login'][0]);
        if(empty($acc))
            header("location:../Admin/Error.php");
        else
            $orderlist=$order->selectAllOrdersByIdUser($_SESSION['login'][0]);
    }
    else
        header("location:../Admin/Error.php");
?>
                                            ...
                                            foreach($orderlist as $order){ 
                                                ...
                                                <tr style="height: 60px;">
                                                    <td class="u-border-1 u-border-grey-15 u-border-no-left u-border-no-right u-first-column u-table-cell">
                                                        <a href="../Users/orderdetails.php?id=<?=$order->getIdOrder()?>"><?=$order->getIdOrder() ?></a>
                                                    </td>
